...
import { addEllipses, decryptData, encryptData, replaceQuotationMarks, shuffleArray } from '../utilities/reuseablefunctions_variables';

const SingleCategory = () => {

    const params = new URLSearchParams(window.location.search);
    const encryptedCategoryID = params.get('category');
    const categoryName = params.get('name');

    const decryptedCategoryID = decryptData(encryptedCategoryID, 'categoryid')

    const [blogPosts, setBlogPosts] = useState(null);
    const [fetchedCategories, setfetchedCategories] = useState(null);
    const [fetchedPagination, setFetchedPagination] = useState([]);
    const endpoint_fetch = '/blog/fetch?perpage=12&category='+decryptedCategoryID; // Replace with your API endpoint
    ...
    const fetchBlogAsync = async (paginationRoute) => {
        ...
        //   const result = await fetchData(!paginationRoute ? endpoint : paginationRoute+"&perpage=2");
          ...
          console.log("Blog Posts For Category ", result);
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                            <div className='col row mb-3'>
                                <h2>Category: <span style={{color:'#fc4600'}}>{!categoryName ? "Loading..." : categoryName}</span></h2>
                            </div>
                            ...
                            <div className="page_pageniation mb-4">
                                <nav aria-label="Page navigation example">
                                    <ul className="pagination justify-content-center">
                                        {!fetchedPagination ? "" : fetchedPagination.length===0 ? "No found data" :  fetchedPagination.map((data, index) => (
